...
Describe 'winver Windows scripts' {
  ...
  It 'dashboard-action.ps1 parses without executing' {
    $tokens = $null
    $errors = $null
    [System.Management.Automation.Language.Parser]::ParseFile((Join-Path $RepoRoot 'windows\dashboard-action.ps1'), [ref]$tokens, [ref]$errors) | Out-Null
    $errors | Should -BeNullOrEmpty
  }

  It 'dashboard-sampler.ps1 parses without executing' {
    $tokens = $null
    $errors = $null
    [System.Management.Automation.Language.Parser]::ParseFile((Join-Path $RepoRoot 'windows\dashboard-sampler.ps1'), [ref]$tokens, [ref]$errors) | Out-Null
    $errors | Should -BeNullOrEmpty
  }
