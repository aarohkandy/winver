...
const { createDashboardServer, parseDashboardArgs } = require("./dashboard");
const { parseJobArgs } = require("./jobs");
...
Usage:
  ...
  winver job <list|start|logs|paths|monitor|pull>
  winver dashboard [--port 8787] [--open]
  winver control <status|server-mode|balanced|reboot|shutdown>
  winver status
  winver server-mode
  winver admin <status|server-profile|lockdown|cooling|unlock|rollback|export-recovery|break-glass> [--dry-run|--apply]
...
async function job(config, args) {
  const options = parseJobArgs(args);
  if (options.mode === "monitor") {
    await monitorJob(config, options);
    return;
  }
  if (options.mode === "pull") {
    await pullJobArchive(config, options);
    return;
  }
  ui.title(`winver job ${options.title}`);
  await sshPowerShell(config, repoScript(config, "windows\\job.ps1", options.psArgs));
}

async function monitorJob(config, options) {
  ui.title(`winver job ${options.title}`);
  let done = false;
  while (!done) {
    const status = await sshPowerShell(
      config,
      repoScript(config, "windows\\job.ps1", ["-Action", "status", "-Target", options.target]),
      { capture: true, allowFailure: true }
    );
    const logs = await sshPowerShell(
      config,
      repoScript(config, "windows\\job.ps1", ["-Action", "logs", "-Target", options.target, "-Tail", String(options.tail)]),
      { capture: true, allowFailure: true }
    );
    ...
    done = /^exit=(?!pending$).+/m.test(status.stdout || "");
    if (!done) await sleep(options.intervalSeconds * 1000);
  }
}

async function pullJobArchive(config, options) {
  ui.title(`winver job ${options.title}`);
  const result = await sshPowerShell(
    config,
    repoScript(config, "windows\\job.ps1", options.psArgs),
    { capture: true, allowFailure: true }
  );
  ...
  if (!fields.archive || !fields.name) throw new Error("Surface did not report an archive path to pull.");
  const localDir = path.resolve(options.localDir || path.join(process.cwd(), "winver-pulls"));
  fs.mkdirSync(localDir, { recursive: true });
  const localPath = path.join(localDir, fields.name);
  const remoteArchive = `${config.host}:${windowsPathForScp(fields.archive)}`;
  await runLocal("scp", ["-O", ...config.sshOptions, remoteArchive, localPath]);
  ui.line("Pulled", localPath, "ok");
}
...
async function admin(config, args) {
  ...
  if (options.profile) psArgs.push("-CoolingProfile", options.profile);
...
async function main(argv) {
  ...
  switch (cmd) {
    ...
    case "logs":
      ...
      break;
    case "job":
      await job(config, args);
      break;
    case "control":
      await control(config, args);
      break;
    case "dashboard":
      await dashboard(config, args);
      break;
    case "admin":
