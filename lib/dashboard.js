  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === "/api/control") {
        await sendControl(config, req, res);
        return;
      }
...
async function sendControl(config, req, res) {
  if (req.method !== "POST" || req.headers["x-winver-dashboard"] !== "1") {
  ...
  if (body.type === "cooling") {
    await sendCoolingControl(config, body.profile, res);
    return;
  }
  if (body.type === "stop-process") {
    await sendStopProcess(config, body.pid, res);
    return;
  }
  if (body.type === "stop-job") {
    await sendStopJob(config, body.target, res);
    return;
  }
...
async function sendCoolingControl(config, profile, res) {
  ...
  if (result.code !== 0) {
    sendJson(res, 502, { ok: false, error: result.stderr || result.stdout || "Could not apply cooling profile." });
    return;
  }
  sendJson(res, 200, { ok: true, text: result.stdout, profile: value });
}

async function sendStopProcess(config, pid, res) {
  ...
  const result = await sshPowerShell(
    config,
    repoScript(config, "windows\\dashboard-action.ps1", ["-Action", "stop-process", "-Pid", String(value)]),
    { capture: true, allowFailure: true, timeoutMs: CONTROL_TIMEOUT_MS }
  );
  sendControlResult(result, res, "Could not stop process.");
}

async function sendStopJob(config, target, res) {
  if (!isSafeJobTarget(target)) {
    sendJson(res, 400, { ok: false, error: "Unsafe job target." });
    return;
  }
  ...
  sendControlResult(result, res, "Could not stop job.");
}

function sendControlResult(result, res, fallback) {
  if (result.code !== 0) {
    sendJson(res, 502, { ok: false, error: result.stderr || result.stdout || fallback });
    return;
  }
  try {
